        ...
        private void Done_Click(object sender, EventArgs e)
        {
           ...
           tests.Add(delegate () { if (JobText.SelectedIndex < 0) { Error("Должность не выбрана"); return true; }; return false; });
            if (CheckTest(tests.ToArray()))
            {
                ...
                if (Id != 0)
                    Controller.UpdateIn(SpecialSqlController.Tables.employeers, new List<string> { LnameText.Text, FnameText.Text, SnameText.Text, BornText.Text, number, AdresText.Text, job, DaysText.Text },Id.ToString());
